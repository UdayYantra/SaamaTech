...
function(record, https, email, search, runtime, url, encode, ui){
	...
	function _inactiveAndSendEmailFun(context, recObj) {
		...
		var approverIdOne = runtime.getCurrentScript().getParameter({name: 'custscript_sam_item_approver_one'});
		var approverIdTwo = runtime.getCurrentScript().getParameter({name: 'custscript_sam_item_approver_two'});

		log.debug({title: 'approverIdOne', details: approverIdOne});

		if(approverIdOne || approverIdTwo) {
			
			//recObj.setValue({fieldId: 'custitem_sam_item_approver', value: approverIdOne});
			recObj.setValue({fieldId: 'isinactive', value: true});
			recObj.setValue({fieldId: 'custitem_sam_item_approval_status', value: 1});

			var approverArr = [];

			if(approverIdOne) approverArr.push(approverIdOne);
			if(approverIdTwo) approverArr.push(approverIdTwo);

			_sendEmailToApprover(approverArr, recObj);
	...
	function _sendEmailToApprover(approverArr, recObj) {

		for(var i=0;i<approverArr.length;i++) {
			
			var emailToId = approverArr[i];
			var userName = 'User';
			var empObj = search.lookupFields({type: search.Type.EMPLOYEE, id: emailToId, columns: ['entityid']});
			...
			var approveURLParam = suiteletURL + '&processFlag=a&recId='+getEncodedValue(recObj.id)+'&recType='+getEncodedValue(recObj.type)+'&aprid='+getEncodedValue(emailToId)+'&aprsts=1';
			var rejectURLParam = suiteletURL + '&processFlag=r&recId='+getEncodedValue(recObj.id)+'&recType='+getEncodedValue(recObj.type)+'&aprid='+getEncodedValue(emailToId)+'&aprsts=1';
	...
	function _showApproveRejectButtons(context, recObj, form) {
		...
		var approvalStatus	= recObj.getValue({fieldId: 'custitem_sam_item_approval_status'});
		var approverIdOne		= recObj.getValue({fieldId: 'custitem_sam_item_approver'});
		var isInactiveVal	= recObj.getValue({fieldId: 'isinactive'});
		var currentUserId	= runtime.getCurrentUser().id;
		
		if(Number(approvalStatus) == 1 && isInactiveVal && Number(approverIdOne) == Number(currentUserId)) {
			
			var suiteletURL = url.resolveScript({scriptId: 'customscript_sam_item_approval_sl', deploymentId: 'customdeploy_sam_item_approval_sl', returnExternalUrl: true});
			
			var approveURLParam	= suiteletURL + '&processFlag=a&recId='+getEncodedValue(recObj.id)+'&recType='+getEncodedValue(recObj.type)+'&aprid='+getEncodedValue(approverIdOne)+'&aprsts=1';
			var rejectURLParam	= suiteletURL + '&processFlag=r&recId='+getEncodedValue(recObj.id)+'&recType='+getEncodedValue(recObj.type)+'&aprid='+getEncodedValue(approverIdOne)+'&aprsts=1';
